...
class LimeExplainer(Explainer):
    ...
    def predict_instance_surrogate(self, instance):
        return np.clip(self.explanation.intercept[1] +
                       sum(weight * ((instance - self.explainer.scaler.mean_) / self.explainer.scaler.scale_)
                       [idx] for idx, weight in self.explanation.local_exp[1]), 0, 1)